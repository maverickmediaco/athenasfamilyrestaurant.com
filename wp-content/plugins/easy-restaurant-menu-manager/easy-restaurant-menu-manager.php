...
function wprmm_admin_menu() {
  if (current_user_can(WPRMM_CAPABILITY)) {
    ...
    add_menu_page($title . "Setup Menus", "Manage Menus", WPRMM_CAPABILITY, WPRMM_ADMIN_URL, "wp_restaurant_admin", "dashicons-list-view");
...
function wprmm_stylesheets() {
  if(!is_admin()){
    ...
    wp_enqueue_style('wprmm-custom-db-style', home_url('/?wprmm-routing=custom-css'), '', '1.5.1');
...
function wprmm_parse_asset($wp) {
    ...
    if (array_key_exists('wprmm-routing', $wp->query_vars) && $wp->query_vars['wprmm-routing'] == 'custom-print-css') {
      $custom_css = stripslashes_deep( get_option('wprmm_custom_print_css') );
      header( 'Content-Type: text/css' );
      echo $custom_css;
      die();exit();
    }
